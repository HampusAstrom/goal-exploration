...
class EvalCallback(EventCallback):
    ...
    def __init__(
        ...
        self.evaluations_initial_value_func: List[List[float]] = []
        self.evaluations_episode_disc_rewards: List[List[float]] = []
    ...
    def _on_step(self) -> bool:
        ...
        if self.eval_freq > 0 and self.n_calls % self.eval_freq == 0:
            ...
            episode_rewards, episode_lengths, initial_values, episode_disc_rewards = evaluate_policy(
                self.model,
                self.eval_env,
            ...
            if self.log_path is not None:
                ...
                self.evaluations_initial_value_func.append(initial_values)
                self.evaluations_episode_disc_rewards.append(episode_disc_rewards)
                ...
                np.savez(
                    ...
                    initial_v=self.evaluations_initial_value_func,
                    disc_f_r=self.evaluations_episode_disc_rewards,
                    **kwargs,
                )
            ...
            self.logger.record("eval/"+self.log_name+"_mean_reward", float(mean_reward))
            ...
            if len(self._is_success_buffer) > 0:
                ...
                self.logger.record("eval/"+self.log_name+"_success_rate", success_rate)

            # Dump log so the evaluation results are printed with the correct timestep
            self.logger.record("time/"+self.log_name+"_total_timesteps", self.num_timesteps, exclude="tensorboard")
...
class MetaEvalCallback(EventCallback):
    ...
    def _init_callback(self) -> None:
        ...
        if self.callback is not None:
            return self.callback.init_callback()

        # TODO add log_path stuff
    ...
    def _on_training_end(self) -> None:
        ...
        if self.log_best and np.abs(self.best_val) == np.inf:
            self.logger.record(self.log_name+"_best", self.best_val)
            self.logger.record(self.log_name+"_best_at_step", np.inf)
            logged_anything = True
